package com.springproject.Edtech.controller;
...
public class AdminController {
	...
	public String userlogin( @RequestParam("username") String username, @RequestParam("password") String pass,Model model) {
		...
		try
		{
			...
			Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/springproject","root","Abhi@gupta12");
	...
	public String removeCategoryDb(@RequestParam("id") int id) {
		try {
			...
			pst.setInt(1, id);
			int i = pst.executeUpdate();
			// Close resources (not done in your code)
			pst.close();
			con.close();
		} catch (Exception e) {
			System.out.println("Exception:" + e);
			// Handle exceptions appropriately (not done in your code)
		}
		return "redirect:/admin/categories";
	}
